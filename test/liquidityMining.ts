...
const liquidityProtectionSettingsAdminAddress =
...
const converterRegistryDataAddress =
  "0x2BF0B9119535a7a5E9a3f8aD1444594845c3A86B";
...
describe("Liquidity mining", function () {
  ...
  before(async function () {
    ...
    const liquidityProtectionSettingsAdminSigner =
      await ethers.provider.getSigner(liquidityProtectionSettingsAdminAddress);
    liquidityProtectionSettingsContract = new ethers.Contract(
      liquidityProtectionSettingsContractAddress,
      liquidityProtectionSettingsAbi.abi,
    ...
    const dappStakingPoolFactory = await ethers.getContractFactory(
      "DappStakingPool",
      addr1
    );
    ...
    [dappBntAnchor] =
      await converterRegistryDataContract.getConvertibleTokenSmartTokens(
    ...
    await dappTokenContract
      .connect(addr1)
      .approve(converterDappBntAddress, ethers.utils.parseEther("10000000000"));
    await bntToken.approve(
      converterDappBntAddress,
    ...
    console.log(
      ...
      }\npool address: ${await upgrades.erc1967.getImplementationAddress(
        dappStakingPoolContract.address
      )}`
    );
    ...
    await dappBntTokenContract
      .connect(addr1)
      .transfer(addr6.address, ethers.utils.parseEther("10"));

    // initiallize 0% for rewards
    funderContract = await upgrades.deployProxy(funderFactory, [
      dappStakingPoolContract.address,
      dappTokenContract.address,
      0,
    ]);
  ...
  it("Should allow claim BNT IL", async function () {
    ...
    await dappTokenContract
      .connect(user)
      .approve(dappStakingPoolContract.address, "30000000000000000000000000");
    await dappStakingPoolContract
      .connect(user)
      .stakeDapp("30000000000000000000000000", 6);
    ...
    await bntTokenContract
      .connect(addr1)
      .approve(
        bancorNetworkContract.address,
  ...
  it("Should allow funding dapp rewards and IL", async function () {
    ...
    const prevDappRewardsSupply =
      await dappStakingPoolContract.dappRewardsSupply();
    const prevFunderBalance = await dappTokenContract.balanceOf(
      funderContract.address
    );
    ...
    expect(prevFunderBalance.toString()).to.equal(
      ethers.utils.parseEther("1000000")
    );
    await dappTokenContract
      .connect(addr1)
    ...
    await dappStakingPoolContract.fund(
      ethers.utils.parseEther("100000"),
      ethers.utils.parseEther("100000")
    );
    const postDappSupply = await dappTokenContract.balanceOf(
      dappStakingPoolContract.address
    );
    const postDappILSupply = await dappStakingPoolContract.dappILSupply();
    const postDappRewardsSupply =
      await dappStakingPoolContract.dappRewardsSupply();
    ...
    const postFundDappSupply = await dappTokenContract.balanceOf(
      dappStakingPoolContract.address
    );
    const postFundDappILSupply = await dappStakingPoolContract.dappILSupply();
    const postFundDappRewardsSupply =
      await dappStakingPoolContract.dappRewardsSupply();
    const postFundFunderBalance = await dappTokenContract.balanceOf(
      funderContract.address
    );
    ...
    expect(postFundDappRewardsSupply).to.equal(
      ethers.utils.parseEther("100000")
    );
  });
  ...
  it("Should update funder contract 44.45% rewards 55.55% IL", async function () {
    ...
    const postFundDappRewardsSupply =
      await dappStakingPoolContract.dappRewardsSupply();
    expect(Number(postFundDappILSupply.sub(prevDappILSupply))).to.equal(
      1000000000000000000000000 * (1 - numerator / denominator)
    );
    expect(
      Number(postFundDappRewardsSupply.sub(prevDappRewardsSupply))
  ...
  it("Should allow transfer position after full unstake", async function () {
    ...
    await dappStakingPoolContract
      .connect(user)
      .stakeDapp(ethers.utils.parseEther("1"), 0);
    ...
    const postPoolEntries = await dappStakingPoolContract.userPoolTotalEntries(
      0
    );
    ...
    const postUnstakePoolEntries =
      await dappStakingPoolContract.userPoolTotalEntries(0);
    ...
    const poolId = await liquidityProtectionContract.callStatic.addLiquidity(
      dappBntAnchor,
      dappTokenContract.address,
      ethers.utils.parseEther("1"),
      { from: user.address }
    );
    await liquidityProtectionContract
      .connect(user)
      .addLiquidity(
        dappBntAnchor,
        dappTokenContract.address,
    ...
    expect(
      userInfo.amount
        .sub(ethers.utils.parseEther("0.006210728081"))
        .abs()
        .lt(ethers.utils.parseEther("0.0000001"))
    ).to.be.true;
    expect(userInfo.lpAmount).to.equal(ethers.utils.parseEther("0"));

    // increase total entries on transfer
    const postTransferPoolEntries =
      await dappStakingPoolContract.userPoolTotalEntries(0);
  ...
  it("Should allow staking one sided dapp", async function () {
    ...
    expect(userInfo.amount).to.equal(
      ethers.utils.parseEther("0.006210728080582889")
    );
    expect(userInfo.lpAmount).to.equal(ethers.utils.parseEther("0"));
  });
  ...
  it("Should allow staking one sided dapp again", async function () {
    ...
    expect(
      userInfo.amount
        .sub(ethers.utils.parseEther("0.012421456161165784"))
        .abs()
        .lt(ethers.utils.parseEther("0.0000001"))
    ).to.be.true;
    expect(userInfo.lpAmount).to.equal(ethers.utils.parseEther("0"));
  ...
  it("Should allow staking DAPP-BNT LP", async function () {
    ...
    await dappStakingPoolContract
      .connect(user)
      .stakeDappBnt(ethers.utils.parseEther("1"), 0);
    userInfo = await dappStakingPoolContract.userPoolInfo(0, user.address);
    let userPostAmount = userInfo.amount;
    expect(userPostAmount.sub(userPreAmount)).to.equal(
      ethers.utils.parseEther("1")
    );
  ...
  it("Should allow transferring positions, notifying, and withdrawing", async function () {
    ...
    expect(
      userInfo.amount
        .sub(ethers.utils.parseEther("0.006210728080582889"))
        .abs()
        .lt(ethers.utils.parseEther("0.0000001"))
    ).to.be.true;
    expect(userInfo.lpAmount).to.equal(ethers.utils.parseEther("0"));
  ...
  it("Should allow users to claim rewards", async function () {
    ...
    await dappStakingPoolContract
      .connect(user)
      .stakeDapp(ethers.utils.parseEther("1"), 6);
    ...
    expect(postDappSupply.sub(preDappSupply).toString()).to.be.above(
  ...
  it("Should allow increase/decrease pool entries double sided stake", async function () {
    ...
    await dappStakingPoolContract
      .connect(user)
      .stakeDappBnt(ethers.utils.parseEther("1"), 6);
    ...
    await dappStakingPoolContract
      .connect(user)
      .unstakeDappBnt("1000000000000000000", 6);
    userInfo = await dappStakingPoolContract.userPoolInfo(6, user.address);
    expect(userInfo.amount).to.equal(ethers.utils.parseEther("0"));
    expect(userInfo.lpAmount).to.equal(ethers.utils.parseEther("0"));
  });
  ...
  it("Should burn pending BNT IL", async function () {
    ...
    await dappTokenContract
      .connect(user)
      .approve(dappStakingPoolContract.address, "18000000000000000000000000");
    await dappStakingPoolContract
      .connect(user)
      .stakeDapp("18000000000000000000000000", 6);

    // simulate IL
    await bntTokenContract
      .connect(addr1)
      .approve(bancorNetworkContract.address, "65400008911974684310");
    const conversionPath = await bancorNetworkContract.conversionPath(
      bntAddress,
      dappTokenContract.address
    );
  ...
  it("Should allow user to unstake DAPP and DAPP-BNT LP from timelocked pool", async function () {
    ...
    const postUnstakePoolEntries =
      await dappStakingPoolContract.userPoolTotalEntries(6);
    expect(postPoolEntries.sub(postUnstakePoolEntries)).to.equal(1);
  });
